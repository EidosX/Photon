...
void MainWindow::reloadCarousel() {
    ...
    while (i < (size_t) oldWidgets.size() && i < newSelection.size()){
        if (oldWidgets[i]->getPath() != newSelection[i].path
                || oldWidgets[i]->getCrop() != newSelection[i].crop) break;
        ++i;
    }
    ...
    for (size_t j = i; j < newSelection.size(); ++j) {
        auto* w = new PreviewImg(newSelection[j].path, this, newSelection[j].crop);
